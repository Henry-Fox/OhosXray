...
func isSocketFD(fd int) (bool, error) {
	...
	if err := unix.Fstat(fd, &stat); err != nil {
		// HarmonyOS VpnConnection TUN fd often denies Fstat (EACCES/EPERM)
		// even though read/write/fcntl work. Treat as non-socket so the
		// Readv dispatcher is used instead of failing New().
		return false, nil
	}
